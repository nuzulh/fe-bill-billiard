...
export default function AddTableDialog({
  ...
  async function onSubmit(val: z.infer<typeof AddTableSchema>) {
    setLoading(true);
    const result = await Services.tableService.create({
      ...val,
    ...
    toast({
      ...
      description: result.message ?? "Meja berhasil ditambahkan",
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button variant="default">Tambah Meja</Button>
      </DialogTrigger>
      <DialogContent className="max-w-[400px] sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Tambah Meja Billiard</DialogTitle>
          <DialogDescription>
